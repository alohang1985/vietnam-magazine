...
try { require('dotenv').config(); } catch(e) { /* ignore if not installed in environment */ }

const CMS_URL = process.env.CMS_URL || 'http://localhost:1337';
const TOKEN = process.env.CMS_ADMIN_TOKEN;
// AUTO-MOCK: if TEST_MODE=mock OR OPENAI_API_KEY is missing, operate in mock fallback mode
const AUTO_MOCK = process.env.TEST_MODE === 'mock' || !process.env.OPENAI_API_KEY;
const MOCK = AUTO_MOCK;
...
async function createPostInCMS(data) {
  if (MOCK) {
    // Attempt to create via API without token; if fails, write to frontend sitemap as mock published
    try {
      const res = await fetch(`${CMS_URL}/api/posts`, { method: 'POST', headers: { 'Content-Type':'application/json' }, body: JSON.stringify({ data }) });
      const j = await res.json().catch(()=>null);
      if (j && j.data) return j;
      // Treat any non-success as error to trigger fallback
      throw new Error('API create failed');
    } catch (e) {
      // fallback: write mock sitemap entry and return simulated response
      const outDir = path.join(__dirname, '../../frontend-next/public');
      try { if (!require('fs').existsSync(outDir)) require('fs').mkdirSync(outDir, { recursive: true }); } catch(e){}
      const sitemapPath = path.join(outDir, 'sitemap.xml');
      let urls = [];
      if (require('fs').existsSync(sitemapPath)) {
        const xml = require('fs').readFileSync(sitemapPath,'utf8');
        const matches = xml.match(/<loc>(.*?)<\/loc>/g) || [];
        urls = matches.map(m=>m.replace(/<\/?loc>/g,''));
      }
      const url = `${process.env.SITE_BASE_URL||'http://localhost:3000'}/posts/${data.slug}`;
      if (!urls.includes(url)) urls.push(url);
      const xml = `<?xml version="1.0" encoding="UTF-8"?>\n<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n${urls.map(u=>`  <url><loc>${u}</loc></url>`).join('\n')}\n</urlset>`;
      require('fs').writeFileSync(sitemapPath, xml);
      // write mock post record for publish_job to pick up
      const mp = path.join(__dirname, '../mock_posts.json');
      const posts = require('fs').existsSync(mp) ? JSON.parse(require('fs').readFileSync(mp,'utf8')) : [];
      posts.push(Object.assign({ id: 'mock-'+Date.now(), created_at: new Date().toISOString() }, data));
      require('fs').writeFileSync(mp, JSON.stringify(posts, null, 2));
      return { data: { id: 'mock-'+Date.now(), attributes: data } };
    }
  }
...
async function pipelineRun() {
  ...
  const DAILY_COUNT = Number(process.env.DAILY_COUNT || (process.env.TEST_MODE==='mock'?3:1));

  for (let idx=0; idx<DAILY_COUNT; idx++) {
    // choose rotating candidate
