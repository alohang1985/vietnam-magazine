...
async function generate(query, sources, region, topic) {
  ...
  const prompt = "당신의 이름은 '릴리'입니다. 베트남을 사랑하는 20대 여성 여행 블로거 릴리로서 포스팅을 작성하세요." + ' ' + '당신은 베트남 여행 전문 20대 여성 블로거입니다. ' + '주제: ' + query + ' ' + '참고자료: ' + snippets + ' ' + '위 주제로 3000자 이상 여행 블로그 포스팅을 마크다운으로 작성하세요. ' + '이모지 포함, 귀엽고 전문적으로. ' + 'JSON이나 코드블록 없이 본문 텍스트만 출력하세요.';
